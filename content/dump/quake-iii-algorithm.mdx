...
i.e. `16.25 = 1 * 1.625 * 10^1 {:bash}` & `-16.25 = -1 * 1.625 * 10^1 {:bash}`.  
...
```bash
=> log2(number) ≈ M / 2^23 + μ + E - 127 # μ is the error correction 
```

After some rearrangement, we get:

```bash
=> log2(number) ≈ 1 / 2^23 * (M + E * 2^23) + μ - 127
```
...
For a number `x^1 {:bash}`:
    1. Doubling the exponent `x^2 {:bash}` : Squares the number
    2. Halving the exponent `x^(1/2) {:bash}` : Gives the square root of the number
    3. Negating the above `x^-(1/2) {:bash}` : Give the inverse square root of the number

Now, recall that a IEEE 754 format is more or less approximately equal to the logarithm of the number.
So, instead of calculating `x^-(1/2) {:bash}`, we will find `log2(x^-(1/2)) {:bash}`

```bash
Let Δ be the inverse square root of x
...
=> 1 / 2^23 * (M(Δ) + E(Δ) * 2^23) + μ - 127 = -(1/2) * ( 1 / 2^23 * (M(x) + E(x) * 2^23) + μ - 127 )

After some calculations,

=> M(Δ) + E(Δ) * 2^23 = 3/2 * 2^23 * (127 - μ) - 1/2 * (M(x) + E(x) * 2^23)
=> M(Δ) + E(Δ) * 2^23 = 0x5f3759df - 1/2 * (M(x) + E(x) * 2^23)
...
For our case, we have the function `f(y) = 1/y^2 - x {:bash}`, since for `y = 0, f(y) = 1/√x {:bash}`.
The derivative of the function is `f'(y) = -2 / y^3 {:bash}`.
...
So, `y' = y * (3/2 - (x/2 * y * y)) {:bash}` i.e. `y * (threehalfs - (x2 * y * y)) {:bash}`
